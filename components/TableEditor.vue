<script setup lang="ts">
    ...
    const table = useState("table", () => [
        new Array(5).fill(createTableCell()),
        new Array(5).fill(createTableCell()),
        new Array(5).fill(createTableCell()),
        new Array(5).fill(createTableCell()),
        new Array(5).fill(createTableCell()),
    ...
    <div class="flex flex-col gap-5">
        <div class="flex justify-around">
            <div class="flex gap-2 items-center">
                ...
                <input id="columns" v-model="columns" class="input input-bordered" name="rows" type="number" min="1" max="100" placeholder="5" required>
            </div>
            <div class="flex gap-2 items-center">
                <label for="rows">Rows: </label>
                <input id="rows" v-model="rows" class="input input-bordered" name="rows" type="number" min="1" max="100" placeholder="5" required>
            </div>
        </div>

        <div class="p-4 mx-auto bg-base-200 rounded-md shadow-md">
            <table class="">
                <tbody class="[&_td]:border [&_td]:border-neutral [&_td]:p-1.5" v-for="(row, index) in table">
                    <tr v-if="index == 0">
                        <td class="border-none"></td>
                        <td v-for="letter in tableLetters()" class="border-none hover:bg-base-100">{{ letter }}</td>
                    </tr>
                    <tr>
                        <td class="border-none hover:bg-base-100">{{ index + 1 }}</td>
                        <td class="hover:bg-base-100" v-for="data in row">
        ...
        <div class="prose">
            ...
            <button class="btn btn-outline btn-primary mx-auto block">Copy to clipboard</button>
        </div>
    </div>
</template>
