...
watch(columns, (newColumns, oldColumns) => {
    ...
    if (addedCols >= 1) {
        const newCols = Array.from({ length: addedCols }, () =>
            createTableCell(),
        );
...
watch(rows, (newRows, oldRows) => {
    ...
    if (addedRows >= 1) {
        for (let i = 0; i < addedRows; i++) {
            table.value.push(
                Array.from({ length: columns.value }, () => createTableCell()),
            );
        }
    } else if (addedRows < 0) {
...
<template>
    <div class="flex flex-col gap-5 p-5">
        <div
            class="flex flex-col gap-3 justify-center items-center relative p-4 bg-base-200 rounded-md shadow-md"
        >
            <table>
                <tbody
